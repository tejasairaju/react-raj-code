...
const ViewQuestionsList = ({
  ...
  const listChoices = (i) => {
    if (['Dropdown', 'Radio button', 'Multiselect'].indexOf(inputList[i].field_type || '') > -1) {
      return (
        <>
          {(inputList[i]['field_choices'] || []).map((choice) => (
            <li>{choice}</li>
          ))}
        </>
      );
  ...
  return (
    <>
      <div>
        {isShowGuidance && (
          <Modal isShow={!!isShowGuidance} isDisclosureDec={true} closeModal={() => setIsShowGuidance(false)}>
            ...
            <div className='get-textarea-input-container'>
              <div className='create-framework__textarea disclosure-description-screen'>
                {_isEmpty(guidance) ? <p dangerouslySetInnerHTML={{ __html: description }}></p> : <p dangerouslySetInnerHTML={{ __html: guidance }}></p>}
              </div>
            </div>
          </Modal>
        )}
