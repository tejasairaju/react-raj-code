...
const AnswerQuestionsTable = (props) => {
    ...
    if (_toLower(questionItem.field_type) === 'dropdown') {
      return (
        <select
          ...
          onChange={(e) => onChangeAnswerHandler(e, disclosureIndex, questionIndex)}
          value={typeof questionItem.value == 'object' ? questionItem.value.value : questionItem.value}
          className='framework__input'
          name='question-dropdown'
          id='answers-dropdown'
        >
          {(questionItem.field_choices || []).map((choice) => (
            <option
              value={choice}
              defaultValue={typeof questionItem.value == 'object' ? questionItem.value.value : questionItem.value}
              selected={choice === (typeof questionItem.value == 'object' ? questionItem.value.value : questionItem.value) ? 'selected' : null}
            >
              {choice}
            </option>
          ))}
      ...
      return (
        <ul className='assign__categories'>
          {(questionItem.field_choices || []).map((radioVal, i) => (
            <RadioButton
      ...
      return (
        <ul className='assign__categories'>
          {(questionItem.field_choices || []).map((choice, i) => (
            <li
              onClick={() => onClickMultiSelect(choice, disclosureIndex, questionIndex)}
              className={`assign__categories-item ${choice === (typeof questionItem.value == 'object' ? questionItem.value.value : questionItem.value) ? 'active' : ''}`}
            >
              {choice}
            </li>
          ))}
        ...
        if (questionItem['isSelected']) {
          return (
            <>
              ...
              {!_isEmpty(_get(questionItem, 'description', '')) && (
                <>
                  <p className='detalis__information-title'>Guidance Notes</p>
                  <TextArea rows={5} type='text' className='assign__categories' value={_get(questionItem, 'description', '')} disabled />
                </>
              )}
              <p className='detalis__information-title'>Answer</p>
              {getAnswerInputField(questionItem, questionIndex)}
              {!_isEmpty(questionItem.field_unit_values) && (
                <>
                  <p className='detalis__information-title'>Unit</p>
              ...
              <>
                {
                  <UploadEvidence
                    ...
                    getDisclosures={() => {
                      props.getDisclosures();
                      onClickQuestions(questionIndex);
                    }}
                    question={questionItem}
                    reportId={reportId}
                    disclosureId={itemDetails.id}
                    kpiId={questionItem.id}
  ...
  return (
    <li className='detalis__item'>
      {isShowDescription && (
        <Modal isShow={!!isShowDescription} isDisclosureDec={true} closeModal={closePopupModal}>
          <div className='create-options-title'>Guidance</div>
          <div className='get-textarea-input-container'>
            <div className='create-framework__textarea disclosure-description-screen'>
              {_isEmpty(_get(itemDetails, 'metaData[0].value', '')) ? (
                <p dangerouslySetInnerHTML={{ __html: itemDetails.description }}></p>
              ) : (
                <p dangerouslySetInnerHTML={{ __html: _get(itemDetails, 'metaData[0].value', '') }}></p>
              )}
            </div>
          </div>
        </Modal>
      ...
      <div className='detalis__top-item' onClick={() => setIsOpenQAcard(!isOpenQAcard)}>
        <div className='details__item-wrapper'>
          ...
          <a
            onClick={(e) => {
              setIsOpenReAssign(!isOpenReAssign);
              e.preventDefault();
              e.stopPropagation();
            }}
            className='detalis__reassign'
          >
            Reassign
        ...
        <div className='details__item-wrapper'>
          <p className='assign__categories-item active'>{itemDetails.category}</p>
        </div>
      </div>
      {isOpenQAcard && (
        <div className='detalis__information'>
          <ul className='assign__categories details__categories'>
            {(questionsList || []).map((question, i) => (
              <li onClick={() => onClickQuestions(i)} className={`assign__categories-item text-overflow-control ${question['isSelected'] ? 'active' : null}`}>
                {question.label}
              </li>
            ))}
          </ul>
          <div className='details__line'></div>
          <div className='details__categories-information size-of-office-space'>{renderAnswerComponent()}</div>
        </div>
      )}
    </li>
  );
