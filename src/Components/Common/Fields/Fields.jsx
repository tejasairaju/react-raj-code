...
const Input = (props) => (
    <>
        {props.label && <h1 className={props.labelCls || 'create-framework__title'}>{props.label}{props.required && <span className="color-red P-4">*</span>}</h1>}
...
const UploadFile = (props) => (
    <>
        ...
        <form className="add__logo-form">
            ...
            {props.logoSizeError&&<label className="logo-size-error"><span>* File size should not exceed 1mb.</span>
            </label>}
        </form></>);
...
const InputBox = (props) => {
    const { type = 'text', name = 'name', value = "", onChangeHandler = () => { }, className = "framework__input", placeholder = "", required = false, disabled = false, maxLength = '' } = props;
    return (<input
        type={type}
        name={name}
        value={value}
...
const TextAreaBox = (props) => (<textarea
    {...props}
    value={props.value}
    onChange={props.onChangeHandler}
    name={props.name}
