...
const QuestionsTable = ({ isAddRemoveButton= false, tableData= null, tableHeader = null, handleInputChange=() => {}, handleRemoveClick = () => {}, handleAddClick =() => {}, isError=false}) => {
    const { login = {}, loginDetails = {} } = useSelector(state => state.signup);
        ...
        <tbody>
            {(tableData || []).map((x, i) => {
                return (<tr>
                    ...
                    <td>
                        <TextArea isEditable={true} label='' inputblockcls={'w-9-em'} name='label' value={x.label} className={`create-framework__input question-title create-framework__textarea ${loginDetails.user_role === 'client_admin' ? 'question-client-user': null}`} placeholder="" required={true} onChangeHandler={(e) => handleInputChange(e, i)} />
                    </td>
                    <td>
                        <Dropdown isEditable={true} className_1={`questions__select select1 ${loginDetails.user_role === 'client_admin' ? 'question-client-user': null}`} className_2={'questions__select-item'} options={dataType} name='type' value={x.type} onChangeHandler={(e) => handleInputChange(e, i)} />
                    </td>
                    <td>
