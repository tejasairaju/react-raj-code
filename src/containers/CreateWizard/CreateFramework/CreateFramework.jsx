    ...
    try {
      ...
      !_isEmpty(frameDetails.logo) &&
        setUploadImage({
          ...
          imageUrl: frameDetails.logo
        });
      updateSubSectorList(frameDetails.supported_sectors);
      setInputValue({
        ...frameDetails,
        categories: frameDetails.supported_category,
        countries: frameDetails.supported_countries,
        sectors: frameDetails.supported_sectors,
        subsectors: frameDetails.supported_sub_sectors
      });
      ...
      if (
        ...
        try {
          ...
          if (isEdit) {
            ...
            response = await axios
              .post(`${process.env.API_BASE_URL}/esgadmin/frameworks`, form, {
                headers: { 'Content-Type': 'multipart/form-data' }
              })
              .then(({ data }) => data);
          }
